...
def process_folder(folder_path, processed_files, log_path):
    ...
    for item in os.listdir(folder_path):
        ...
        if os.path.isdir(item_path):
            ...
            try:
                if correct_ext in ['txt', 'log', 'md', 'conf', 'cfg', 'ini', 'css']:
                ...
                elif correct_ext == 'docx':
                    data = process_docx_file(item_path)
                    
                elif correct_ext in ['html', 'htm', 'xml']:
                    data = process_html_xml_file(item_path)
                    
                elif correct_ext == 'json':
                    data = process_json_file(item_path)
    ...
    try:
        ...
        for table in doc.tables:
            ...
            for row in table.rows:
                ...
                # 简单去重，因为合并单元格可能导致重复读取
                # 这里简单处理：只要有内容就加进去
                if any(row_cells):
                    # 对表格内容也做一次清洗，保持与 excel 处理逻辑一致
                    cleaned_row = [str(cell).strip() for cell in row_cells]
                    data.append(cleaned_row)
                    
    except Exception as e:
        log_message(f"DOCX 读取失败: {e}")
        raise
    return data

def process_html_xml_file(file_path):
    """使用 BeautifulSoup 提取 HTML/XML 中的纯文本"""
    data = []
    try:
        with open(file_path, 'r', encoding='utf-8', errors='replace') as f:
            content = f.read()
            
        # 使用 lxml 解析器，如果没安装可以使用 'html.parser'
        soup = BeautifulSoup(content, 'html.parser')
        
        # 获取纯文本，去除标签
        # get_text(separator=' ') 会用空格代替标签换行
        ...
        # 降级策略：如果解析失败，尝试直接当纯文本读
        log_message(f"HTML/XML 解析异常，尝试作为纯文本处理: {e}")
...
def process_json_file(file_path):
    """
    处理 JSON 文件。
    策略：尝试解析 JSON 提取所有 value，如果不是标准 JSON 或者是 JSONL，则回退到文本处理。
    """
    data = []
    ...
    try:
        ...
        try:
            ...
            # 提取所有叶子节点的值，展平成一行或多行
            # 这里简单处理：将整个 JSON 提取出的值视为“一行”或“多行”
            # 考虑到 JSON 结构复杂，最好的方式是把提取到的所有 value 当作文本行进行分词
            
            extracted_raw = []
            extract_values(json_obj, extracted_raw)
            
            # 方案：把提取出的所有值拼接成一个大字符串，然后按行切割（如果有换行符），或者直接分词
            # 为了适配你的 identify_special_fields，我们需要构造类似 [[col1, col2, col3]] 的结构
            
            # 简化策略：将提取到的所有值，每行放一个，或者直接按文本处理
            # 鉴于 JSON 可能是 [{row1}, {row2}] 格式
            if isinstance(json_obj, list):
                for item in json_obj:
                    row_vals = []
                    extract_values(item, row_vals)
                    if row_vals:
                         data.append(row_vals) # 保持列表结构，不需 tokenize_text，因为已经是分离的值
            else:
                 # 单个对象
                row_vals = []
                extract_values(json_obj, row_vals)
                if row_vals:
                    data.append(row_vals)
                    
        except json.JSONDecodeError:
            # 如果不是标准 JSON（可能是 JSON Lines 或 损坏的 JSON），按纯文本处理
            return process_txt_file(file_path)
            
    except Exception as e:
        log_message(f"JSON 处理失败: {e}")
        return process_txt_file(file_path)
