...
def build_setparams_context(
    ...
    return {
        ...
        "darknet_filters": normalize_filter_rows(darknet_filters or []),
...
def darknet_view(request):
    ...
    stats_cache = get_cached_stats(stats_cache_key)
    cached_distinct_count = stats_cache.get("distinct_count")
    final_distinct_count = max(0, cached_distinct_count - 1) if cached_distinct_count else None
    embed_api_base_url = get_embed_api_base_url(request)
    ...
    return render(
        ...
        "db_display/darknet.html",
        {
            ...
            "next_cursor_json": get_next_cursor_json(next_cursor),
            "distinct_root_domains_count": final_distinct_count,
            "stats_updated_at": stats_cache.get("updated_at"),
