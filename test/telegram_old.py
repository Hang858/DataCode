...
GLOBAL_CONFIG = {
    ...
    "OPENSEARCH_INDEX": "telegram_old_test_index",
    "API_RATE_LIMIT": 10,
...
def get_existing_telegram_count(os_client):
    try:
        response = os_client.count(index=GLOBAL_CONFIG["OPENSEARCH_INDEX"])
        return int(response.get("count", 0))
    except Exception as exc:
        logging.warning(f"查询OpenSearch telegram文档数量失败: {exc}")
        return 0
...
def align_telegram_id_map_auto_increment(cursor, conn, os_client):
    ...
    existing_count = get_existing_telegram_count(os_client)
    next_id = max(int(map_max_id), existing_count) + 1
    if next_id <= 1:
        return
    ...
    logging.info(
        ...
        "(map_count=%s, map_max_id=%s, opensearch_count=%s)",
        next_id,
        map_count,
        map_max_id,
        existing_count,
    ...
    index_body = {
        ...
        "mappings": {
            ...
            "properties": {
                ...
                "chat_name": {"type": "text", "analyzer": "standard"},
                "org": {"type": "text", "analyzer": "standard"},
                "sender_first_name": {"type": "text", "analyzer": "standard"},
                "sender_last_name": {"type": "text", "analyzer": "standard"},
                "event": {"type": "text", "analyzer": "standard"},
